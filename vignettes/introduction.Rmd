---
title: "An introduction to the R package 'animate'"
...
To include the visualisation in an RMarkdown Document, 

- first export the visualisation to a JSON file using the `device$export` function,
- then use `animate::insert_animate` to insert the visualisation in a code chunk.
...
- at the root of the shiny app folder, run `setup_animate('./www')` to copy the necessary asset files to the 'www' folder,
- call `load_animate` in the `ui` to include the asset files, and
- then use the device function in the `server` as usual inside `shiny::observeEvent`.

Here is a full example:

```{r}
# At the root of the shiny app folder, run `setup_animate('./www')` to copy the
# necessary files to the 'www' folder
library(shiny)
library(animate)

ui <- fluidPage(
  load_animate(),  # Include the asset files
